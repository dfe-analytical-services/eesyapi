...
Whilst the above are best used for automated pipeline workflows, the EES API also has an connection
...
The IDs are intended to aid in future proofing fir users that may develop automated pipelines that
connect to the EES API. Labeling inevitably changes, for example local authorities sometimes get
renamed or column labels are updated to be clearer. If the underlying content of that item stays
the same, i.e. the renamed local authority still has the same boundaries, then automated pipelines
will be required to still connect to the same item, therefore each item has an associated ID that
will remain unchanged in a given data set as it is updated with new data over the subsequent
releases, even if a renaming of a field occurs.
