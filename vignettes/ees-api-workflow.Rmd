...
**`r example_id("publication", group = "attendance")`**. We can now use this in the function 
...
```
eesyapi::preview_dataset("57b69201-033a-2c77-a19f-abcce2b11341")
```
...
**`r example_id(group = "attendance")`** and provide it as a parameter to `get_meta`:
...
In this example, let's say we've run `eesyapi::get_meta("57b69201-033a-2c77-a19f-abcce2b11341")`
...
```{r echo=FALSE}
meta_data <- eesyapi::get_meta(dataset_id = "57b69201-033a-2c77-a19f-abcce2b11341")
time_period_example <- meta_data |>
  magrittr::extract2("time_periods") |>
  dplyr::filter(
    label %in% c("2024 Week 24")
...
filter_columns_3 <- meta_data|>
  ...
  dplyr::filter(col_name %in% c("attendance_reason", "day_number")) |>
  dplyr::pull("col_id")
filter_example_3 <- meta_data |>
  magrittr::extract2("filter_items") |>
  dplyr::filter(item_label == "Total", col_id %in% c(filter_columns_3))|> 
  dplyr::pull("item_id")|> 
  paste(collapse = "\" and \"")
```
...
- "`r filter_example_2`"
- "`r filter_example_3`"
- "`r example_id("indicator",group = "attendance")`"
...
in the **Absences** category, **Nationally** for **York** in **Week 24** in **2024**, we would 
